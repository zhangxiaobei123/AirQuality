...
object GridInferMonthTwoApp {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      s"""
        |select * from dwd_air.dwd_grid_infer_middle_two where substring(dt, 1, 7) = '${date}'
        |""".stripMargin)
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_grid_infer_evaluate_month_two partition(dt='${date}')
