package com.glzt.prot.evaluate.grid_51.history
...
object GridMonth2TiDB51App_his {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val query: String =
      s"""
         ...
         |  AND substring(published_at, 1, 7) = '${date}'
         |) t
         |""".stripMargin
    ...
    val query2: String =
      s"""
         ...
         |  AND substring(forecast_at, 1, 7) = '${date}'
         |) t
         |""".stripMargin
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_grid_forecast_evaluate_month_cd partition(dt='${date}')
    ...
    df5.na.fill(-99).foreachPartition((partition: Iterator[Row]) => {
      ...
      try {
        ...
        val sql = "REPLACE INTO `grid_forecast_evaluate_month_cd`(time_type, year, month, control_area_id, forecast_interval, " +
