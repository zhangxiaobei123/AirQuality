package com.glzt.prot.evaluate.whole_two

import com.alibaba.fastjson.JSON
import com.glzt.prot.utils.Func._
import com.glzt.prot.utils.{JDBCUtils, TwoWholeData}
import org.apache.spark.sql.{Row, SparkSession}

import java.sql.{Connection, PreparedStatement}

object WholeQuarterTwoApp {
  def main(args: Array[String]): Unit = {
    ...
    val query: String =
      s"""
         ...
         |  b.DATA AS real_data
         |FROM
         |  `alpha-center`.whole_area_forecast_day_air_quality_cd a join `alpha-center`.whole_area_day_air_quality_cd b
         |on
         |  b.source = 'monitoring'
         |  AND a.control_area_id in (5, 7, 10, 12, 19, 25)
    ...
    val df2 = df.dropDuplicates().na.drop("any").rdd.map(x => {
      ...
      TwoWholeData(control_area_id, time_step,
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_whole_forecast_evaluate_quarter_two partition(dt='${year}_${lastQuarter}')
         |select * from result_view
         |""".stripMargin)

    //todo:统计结果推送到TiDB
    df3.na.fill(-99).foreachPartition((partition: Iterator[Row]) => {
