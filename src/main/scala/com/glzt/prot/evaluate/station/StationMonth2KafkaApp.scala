...
object StationMonth2KafkaApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    println(df.dropDuplicates().na.drop("any").count() + "-------" + df.rdd.getNumPartitions)
    //    df.printSchema()
...
//      "pm2_5r" :: "pm10r" :: "o3r" :: "cor" :: "no2r" :: "so2r" :: Nil, Map("null" -> "0"))
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_station_forecast_evaluate_month_lq partition(dt='${date}')
    ...
    df3.na.fill(0)
      ...
      try {
        ...
        val sql = "REPLACE INTO `station_forecast_evaluate_month_lq`(time_type, year, month, station_id, " +
