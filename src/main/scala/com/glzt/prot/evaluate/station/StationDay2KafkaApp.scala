...
//      val station_id = x.getInt(0).toString
//      val published_at = x.getTimestamp(1).toString.substring(0, 19)
...
//      ForecastData(station_id, published_at,
...
//      "pm2_5r" :: "pm10r" :: "o3r" :: "cor" :: "no2r" :: "so2r" :: Nil, Map("null" -> "0"))
    ...
    val df4 = spark.sql(
      s"""
        ...
        |accuracy from dwm_air.dwm_station_forecast_evaluate_month_lq
        |union all
        |select
        |station_id                    ,
        |pm2_5_nmb                     ,
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_station_forecast_evaluate_day_lq partition(dt='${day}')
         |select * from day_view
         |""".stripMargin)

    //todo:统计结果推送到TiDB
    df4.na.fill(0).foreachPartition((partition: Iterator[Row]) => {
      ...
      try {
        ...
        val sql = "REPLACE INTO `station_forecast_evaluate_day_lq`(time_type, day, station_id, " +
        ...
        partition.foreach(x => {
          ...
          pstmt.setInt(3, x.getString(2).toInt)
