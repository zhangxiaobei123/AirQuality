...
import com.glzt.prot.utils.Func.{getLastQuarterMonth, grade_evaluate_interval, grade_evaluate_percent, primary_pollutant_accuracy}
import com.glzt.prot.utils.JDBCUtils
import org.apache.spark.sql.{Row, SparkSession}

import java.sql.{Connection, PreparedStatement}

object GridInferQuarter2TiDB51App {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      s"""
        |select * from dwd_air.dwd_grid_infer_middle_51 where substring(dt, 1, 7) in (${date})
        |""".stripMargin)
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_grid_infer_evaluate_quarter_cd partition(dt='${year}_${lastQuarter}')
