package com.glzt.prot.evaluate.gridinfer_51
...
object gridInferMidData51App {
  def main(args: Array[String]): Unit = {
    ...
    val df_grid = spark.read.format("csv")
      .option("header", "true")
      .option("sep", ",")
      .load("hdfs://hacluster/dolphinscheduler/root/resources/yzhen/lib/control_area_grid.csv")

      df_grid.createOrReplaceTempView("view_51")

    spark.sql(
      """
        |select a.* from real_tmp a join view_51 b on a.grid_id = b.grid_id and b.control_area_id in (26, 17)
    ...
    spark.sql(
      s"""
         |insert overwrite table dwd_air.dwd_grid_infer_middle_51 partition(dt='${day}')
