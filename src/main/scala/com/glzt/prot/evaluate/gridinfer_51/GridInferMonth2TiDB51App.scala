...
object GridInferMonth2TiDB51App {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      s"""
        |select * from dwd_air.dwd_grid_infer_middle_51 where substring(dt, 1, 7) = '${date}'
        |""".stripMargin)
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_grid_infer_evaluate_month_cd partition(dt='${date}')
    ...
    df.na.fill(0).foreachPartition((partition: Iterator[Row]) => {
      ...
      try {
        ...
        val sql = "REPLACE INTO `grid_infer_evaluate_month_cd`(time_type, year, month, control_area_id," +
