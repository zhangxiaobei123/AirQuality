...
object GridInferDay2TiDB51App {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      """
        |select * from dwd_air.dwd_grid_infer_middle_51
        |""".stripMargin)
    ...
    spark.sql(
      s"""
         |insert overwrite table dwm_air.dwm_grid_infer_evaluate_day_cd partition(dt='${day}')
