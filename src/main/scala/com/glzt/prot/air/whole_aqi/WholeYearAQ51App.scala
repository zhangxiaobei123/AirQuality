  ...
  def main(args: Array[String]): Unit = {
    ...
    val query_inference: String =
      s"""
         ...
         |  if(sum(json_extract(`data`, '$$.pm2_5_flag'))>=322,1,0) as pm2_5_flag,
         |  if(sum(json_extract(`data`, '$$.pm10_flag'))>=322,1,0) as pm10_flag,
         |  if(sum(json_extract(`data`, '$$.o3_flag'))>=322,1,0) as o3_flag,
         |  if(sum(json_extract(`data`, '$$.co_flag'))>=322,1,0) as co_flag,
         |  if(sum(json_extract(`data`, '$$.no2_flag'))>=322,1,0) as no2_flag,
         |  if(sum(json_extract(`data`, '$$.so2_flag'))>=322,1,0) as so2_flag,
         ...
         |where substring(published_at, 1, 4) = '${year}' group by source, control_area_id
         |) tab
         |""".stripMargin
    ...
    val df_inference = spark.read.format("jdbc")
      ...
      .option("dbtable", query_inference)
    ...
    df_inference.na.fill(0).dropDuplicates().createOrReplaceTempView("year_view1")
    ...
    df_inference2.withColumn("data", to_json(struct(
      ...
      .coalesce(1).createOrReplaceTempView("year_view2")
    ...
    spark.sql(
      ...
      try {
        ...
        val sql = "REPLACE INTO `whole_area_year_air_quality_cd`(source, data, published_year, control_area_id) VALUES(?, ?, ?, ?)"
