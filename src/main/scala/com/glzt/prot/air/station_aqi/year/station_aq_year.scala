package com.glzt.prot.air.station_aqi.year


import com.glzt.prot.utils.AQI.{calc_iaqi_co_24h, calc_iaqi_no2_24h, calc_iaqi_o3_8h, calc_iaqi_pm10_24h, calc_iaqi_pm2_5_24h, calc_iaqi_so2_24h, get_aqi, get_co_index, get_grade, get_o3_index, get_primary_pollutants}
import org.apache.spark.sql.{Row, SparkSession}
import org.apache.spark.sql.expressions.Window
import org.apache.spark.sql.functions.{avg, ceil, col, collect_list, count, from_json, lit, round, struct, to_json}
import org.apache.spark.sql.types.{DoubleType, IntegerType, StructType}

import java.sql.DriverManager
import java.util.Properties

object station_aq_year {
  def main(args: Array[String]): Unit = {
    ...
    if (args.length != 1) {
      ...
      System.err.println("Usage: station_aq_year <publish_year>,args参数个数为：" + args.length)
      System.exit(-1)
    }

    val Array(publish_year) = args

    val start_date = publish_year + "-01-01"
    val end_date = publish_year + "-12-31"
    ...
    val source_station_day_data = spark.read.jdbc(center_url, "fixed_station_day_aq", props)
      ...
      .where(col("published_at").>=(start_date) && col("published_at").<=(end_date))
      .withColumn("data", from_json(col("data"), schema))
      .select("station_id", "data.co", "data.so2", "data.pm2_5", "data.pm10", "data.no2", "data.o3", "data.grade", "published_at")

    source_station_day_data
      .withColumn("count", count(col("published_at")).over(Window.partitionBy("station_id")))
      ...
      .withColumn("co_index", get_co_index(collect_list(col("co")).over(Window.partitionBy("station_id")), col("count")))
      .withColumn("o3_index", get_o3_index(collect_list(col("o3")).over(Window.partitionBy("station_id")), col("count")))
      ...
      .withColumn("published_year", lit(publish_year))
      ...
      .select("station_id", "published_year", "comprehensive_index", "co", "o3", "pm10", "pm2_5", "so2", "no2", "so2_iaqi", "pm10_iaqi", "pm2_5_iaqi", "o3_iaqi", "no2_iaqi", "co_iaqi", "aqi", "grade", "primary_pollutants")
      .withColumn("data", to_json(struct(col("comprehensive_index"), col("aqi"), col("no2_iaqi"), col("so2_iaqi"), col("co_iaqi"), col("o3_iaqi"), col("pm2_5_iaqi"), col("pm10_iaqi"), col("grade"), col("primary_pollutants"), col("no2"), col("so2"), col("co"), col("o3"), col("pm2_5"), col("pm10")), Map("ignoreNullFields" -> "true")))
      .select("station_id", "data", "published_year")
      .withColumn("published_year", col("published_year").cast(IntegerType))
      .foreachPartition((dataList: Iterator[Row]) => {
        ...
        dataList.foreach(data => {
          stmt.addBatch("insert into fixed_station_year_aq(station_id,data,published_year) values('" + data.getInt(0) + "','" + data.getString(1) + "','" + data.getInt(2) + "') on duplicate key update data=values(data)")
        })
        stmt.executeBatch()
