...
/**
 * @Author:Tancongjian
 * @Date:Created in 9:47 2022/1/18
 *
 */
object JniuJjiangStation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //        val start = "2022-06-08 10:00:00"
    //        val end = "2022-06-08 10:00:00"

    val spark = SparkSession
      .builder()
      .appName("金牛锦江区控数据入hive")
              .master("local[*]")
...
//    .option("url", "jdbc:mysql://117.50.24.184:4000/alpha-center?Unicode=true&characterEncoding=utf-8&useSSL=false")
      ...
      .option("dbtable", "aq_fixed_station")
        ...
        val jinniudata = getData(start, end,"510106")
        val jinniu = josn_data(jinniudata)

        val jingjiangdata = getData(start, end,"510104")
        val jingjiang = josn_data(jingjiangdata)

        val data = jinniu.unionAll(jingjiang)

        data.show()
        datafarme_insert(data)
