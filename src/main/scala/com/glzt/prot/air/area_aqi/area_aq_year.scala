...
object area_aq_year {
  def main(args: Array[String]): Unit = {
    ...
    if (args.length != 1) {
      ...
      System.err.println("Usage: area_aq_year <publish_year>,args参数个数为：" + args.length)
      System.exit(-1)
    }
    ...
    val source_area_day_data = spark.read.jdbc(center_url, "area_day_air_quality_lq", props)
    ...
    source_area_day_data
      ...
      .withColumn("co_index", get_co_index(collect_list(col("co")).over(Window.partitionBy("area_id", "source")), col("count")))
      .withColumn("o3_index", get_o3_index(collect_list(col("o3")).over(Window.partitionBy("area_id", "source")), col("count")))
      ...
      .select("source", "area_id", "source", "published_year", "comprehensive_index", "co", "o3", "pm10", "pm2_5", "so2", "no2", "so2_iaqi", "pm10_iaqi", "pm2_5_iaqi", "o3_iaqi", "no2_iaqi", "co_iaqi", "aqi", "grade", "primary_pollutants")
      ...
      .select("source", "area_id", "data", "published_year")
      ...
      .foreachPartition((dataList: Iterator[Row]) => {
        ...
        dataList.foreach(data => {
          stmt.addBatch("insert into area_year_air_quality_lq(source,area_id,data,published_year) values('" + data.getString(0) + "','" + data.getInt(1) + "','" + data.getString(2) + "','" + data.getInt(3) + "') on duplicate key update data=values(data)")
        })
        stmt.executeBatch()
