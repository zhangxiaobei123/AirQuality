...
object area_aq_day {
  def main(args: Array[String]): Unit = {
    ...
    val Array(publish_date, end_hour) = args
    ...
    val area_grid = spark.read.jdbc(url, "area_grid", props)
      .select("area_id", "grid_id")
      .join(broadcast(area_control_area), area_control_area.col("area_id_tmp").===(col("area_id")))
      .select("grid_id", "area_id", "control_area_id").withColumnRenamed("grid_id", "grid_id_tmp")
      .withColumn("count", count("grid_id_tmp").over(Window.partitionBy("area_id")))
      .withColumnRenamed("control_area_id", "control_area_id_tmp")

    val station_area = spark.read.jdbc(url, "aq_fixed_station_grid", props)
      .select("grid_id", "station_id")
      .join(area_grid.filter(col("area_id").=!=(1492))
        ...
        .filter(col("area_id").=!=(60374)), col("grid_id_tmp").===(col("grid_id")))
      .select("grid_id", "area_id", "station_id")
      .withColumn("count", count("station_id").over(Window.partitionBy("area_id")).cast(DoubleType))
      .withColumnRenamed("station_id", "station_id_tmp")

    /**
     * GB 3095-2012文件的6.1.4小结中规定监测点数量不能低于城市总数量的0.75（总数量小于4时，不低于0.5）。如果不满足就标记为无效数据
     * (pointNum < 4 && (values.length / pointNum) < 0.5) || ((pointNum >= 4) && (values.length / pointNum) < 0.75)
     * values 是每个区域下的网格或者站点参与计算的值的集合 然后pointNum是该区域下实际的站点个数和网格个数
     */

    val area_aq_day_monitor_data_ = spark.read.jdbc(url, "fixed_station_day_aq", props)
      .select("station_id", "data", "published_at")
      .filter(col("published_at").===(publish_date))
      .join(broadcast(station_area), station_area.col("station_id_tmp").===(col("station_id")))
      .withColumn("data", from_json(col("data"), schema))
      .select("station_id", "area_id", "data.co", "data.so2", "data.pm2_5", "data.pm10", "data.no2", "data.o3", "published_at", "count")
      .withColumn("pm10", avg(col("pm10")).over(Window.partitionBy("area_id"))) //pm10_24h的浓度计算
      .withColumn("pm2_5", avg(col("pm2_5")).over(Window.partitionBy("area_id"))) //pm2_5_24h的浓度计算
      .withColumn("co", avg(col("co")).over(Window.partitionBy("area_id"))) //pm2_5_24h的浓度计算
      .withColumn("no2", avg(col("no2")).over(Window.partitionBy("area_id"))) //pm2_5_24h的浓度计算
      .withColumn("so2", avg(col("so2")).over(Window.partitionBy("area_id"))) //pm2_5_24h的浓度计算
      .withColumn("o3", avg(col("o3")).over(Window.partitionBy("area_id"))) //o3_8h的浓度计算
      .dropDuplicates("area_id", "station_id")
      .withColumn("area_count", count("station_id").over(Window.partitionBy("area_id")).cast(DoubleType)) //
      .withColumn("account", round(lit(col("area_count")./(col("count"))), 1))
      .withColumn("valid", when((col("count").<(4) && (col("account")).<(0.5)) || (col("count").>=(4) && col("account").<(0.75)), 0).otherwise(1))
      .drop("account", "station_id", "area_count", "count")
      ...
      .withColumn("data", to_json(struct(col("valid"), col("aqi"), col("no2_iaqi"), col("so2_iaqi"), col("co_iaqi"), col("o3_iaqi"), col("pm2_5_iaqi"), col("pm10_iaqi"), col("grade"), col("primary_pollutants"), col("no2"), col("so2"), col("co"), col("o3"), col("pm2_5"), col("pm10"), col("so2_flag"), col("co_flag"), col("o3_flag"), col("no2_flag"), col("pm10_flag"), col("pm2_5_flag"), col("original")), Map("ignoreNullFields" -> "true")))
      .dropDuplicates("area_id")
      .select("source", "area_id", "data", "published_at")

    val whole_area_day_air_quality_cd_tb_name = s"(select source,data,published_at,control_area_id from whole_area_day_air_quality_cd where create_time between '$start_hour' and '$end_hour' and source='monitoring' and control_area_id in (10,25,13,7,12,17,19,16,14,15,11,5,22)) as whole_area_day_air_quality_cd_filtered"
    val area_aq_day_monitor_data = spark.read.format("jdbc")
      ...
      .option("dbtable", whole_area_day_air_quality_cd_tb_name)
      ...
      .select("source", "area_id", "data", "published_at")
      .union(area_aq_day_monitor_data_)

    val grid_day_aq_cd_tb_name = s"(select control_area_id,grid_id,data,published_at from grid_day_aq_cd where published_at='$publish_date') as grid_day_aq_cd_filtered"

    //inference模式计算
    val grid_day_aq = spark.read.format("jdbc")
      .option("url", url)
      .option("driver", "com.mysql.jdbc.Driver")
      .option("dbtable", grid_day_aq_cd_tb_name)
      .option("user", "glzt-pro-bigdata")
      .option("password", "Uhh4QxUwiMsQ4mK4").load()

    do_area_day_compute(grid_day_aq, area_grid, publish_date, area_aq_day_monitor_data, schema)
      .foreachPartition((dataList: Iterator[Row]) => {
        ...
        //预备语句
        val sql = "insert into area_day_air_quality_cd(source,area_id,data,published_at) values(?,?,?,?) on duplicate key update data=values(data)"
        //预备语句
  ...
  def do_area_day_compute(source_data: DataFrame, area_grid: DataFrame, publish_date: String, area_aq_day_monitor_data: DataFrame, schema: StructType) = {
    source_data.withColumn("data", from_json(col("data"), schema))
      .select("control_area_id", "grid_id", "data.co", "data.so2", "data.pm2_5", "data.pm10", "data.no2", "data.o3", "published_at")
      .join(broadcast(area_grid), area_grid.col("control_area_id_tmp").===(col("control_area_id")) && area_grid.col("grid_id_tmp").===(col("grid_id")))
      ...
      .dropDuplicates("area_id", "grid_id")
      .withColumn("area_count", count(col("grid_id")).over(Window.partitionBy("area_id")).cast(DoubleType))
      .withColumn("account", round(lit(col("area_count")./(col("count"))), 2))
      .withColumn("valid", when((col("count").<(4) && (col("account")).<(0.5)) || (col("count").>=(4) && col("account").<(0.75)), 0).otherwise(1))
      .withColumn("published_at", lit(publish_date))
      ...
      .dropDuplicates("area_id")
      .select("source", "area_id", "data", "published_at")
      .union(area_aq_day_monitor_data)
  }
}
