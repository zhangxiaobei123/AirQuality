...
/**
 * @Author:Tancongjian
 * @Date:Created in 9:49 2023/2/27
 *
 */
object GridForecast1hourWeather {
...
//      .master("local[*]")
      ...
      for (grid <- grids) {
        ...
        loop.breakable {
          for (i <- 1 to 15) {
            ...
            var sign = "fields=weather_hourly_1h&locations=" + append + "&public_key=" + PUBLIC_KEY + "&ts=" + hourTime + "&ttl=1200"
            var encode = genHMAC(sign)
            var requestUrl = URL + "fields=weather_hourly_1h&public_key=" + PUBLIC_KEY + "&ts=" + hourTime + "&ttl=1200&locations=" + append + "&sig=" + encode
        ...
        if (result != null) {
          ...
          val weatherHourly1h = jsonObject.getJSONArray("weather_hourly_1h")
          if (weatherHourly1h != null) { //遍历weather_hourly_3h
            for (i <- 0 until weatherHourly1h.size) {
              val weatherHourly1hJSONObject: JSONObject = weatherHourly1h.getJSONObject(i)
              val data: JSONArray = weatherHourly1hJSONObject.getJSONArray("data")
              //遍历data
    ...
    getData()
        ...
        val xinzhi_grid_foorecast_1hour_weathers = rdd.flatMap(x => {
          val lines = x.toArray
        ...
        xinzhi_grid_foorecast_1hour_weathers.withColumn("grid_id", col("grid_id").cast(IntegerType))
          ...
          .foreachPartition((dataList:Iterator[Row]) => {
            ...
            try {
              ...
              //构建sql语句
              val sql = "insert into xinzhi_grid_forecast_hour_weather(grid_id,grid_row,grid_column,grid_center_coord,data,published_at,forecast_at) values(?,?,?,?,?,?,?) on duplicate key update data=values(data),grid_center_coord=values(grid_center_coord)"
        ...
        xinzhi_grid_foorecast_1hour_weathers.unpersist()
