  ...
  def main(args: Array[String]): Unit = {
    ...
    def josn_data(List_data: util.ArrayList[String]) {
      ...
      if (resultLists.size() == 788) {
        ...
        xinzhi_grid_hour_weathers.show()
        xinzhi_grid_hour_weathers.withColumn("grid_id", col("grid_id").cast(IntegerType))
          ...
          //        .withColumn("grid_scale", col("grid_scale").cast(IntegerType))
    ...
    val tableName = s"(SELECT grid_id,published_at FROM grid_hour_weather where published_at ='$star_time') t"
